...
# For now we disable the heavy PDF quiz RAG pipeline (Torch, transformers, etc.)
# so that the scheduler API can run reliably on your machine.
def run_quiz_pipeline(_pdf_bytes: bytes):
    raise RuntimeError(
        "PDF quiz generation is temporarily disabled on this dev backend; "
        "scheduler endpoints still work."
    )

# Scheduler: modules use bare "import Task", "import Confidence" etc.
# We add study_scheduler to path and create compatibility aliases so
# their capitalised imports work even though files are lowercase.
...
try:
    _confidence_mod = _importlib.import_module("confidence")
    sys.modules.setdefault("Confidence", _confidence_mod)
except Exception:
    pass

try:
    _quizresult_mod = _importlib.import_module("quizResult")
    sys.modules.setdefault("QuizResult", _quizresult_mod)
except Exception:
    pass

try:
    _task_mod = _importlib.import_module("task")
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[FRONTEND_URL, "http://localhost:3000", *ADDITIONAL_DEV_ORIGINS],
